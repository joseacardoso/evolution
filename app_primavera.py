...
def render_primavera() -> None:
    ...
    if selected_core_modules:
        plan_rank = {name: idx for idx, name in enumerate(PLAN_ORDER, start=1)}
        min_plan = max(
            (PRIMAVERA_CORE_MODULE_MIN_PLAN[deployment_key][mod] for mod in selected_core_modules),
            key=lambda p: plan_rank[p],
        )
        st.info(f"Pelos módulos base selecionados, o plano mínimo é: **{min_plan}**.")
    else:
        st.caption("Seleciona módulos base para orientar automaticamente o plano recomendado.")
